...
export async function GET(request, response) {
    ...
    if (data.Response === "False") {
        return response.json({ "message": "Sorry, the film '" + filmTitle + "' was not able to be found." });
    } else {
        console.log("wagwan");
        ...
        if (checkFilm.length > 0) {
            return response.json({ "message": "Sorry, the film '" + data.Title + "' has already been added to the database." });
        } else {
            ...
            return response.json({ "message": "Success! The film '" + data.Title + "' was added to the database." });
        }
    }
}
